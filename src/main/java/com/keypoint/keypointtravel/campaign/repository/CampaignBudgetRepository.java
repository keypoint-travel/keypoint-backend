...
public interface CampaignBudgetRepository extends JpaRepository<CampaignBudget, Long> {
    ...
    @Query("SELECT new com.keypoint.keypointtravel.campaign.dto.dto.TotalBudgetDto(SUM(cb.amount), cb.currency) " +
        "FROM CampaignBudget cb WHERE cb.campaign.id = :campaignId GROUP BY cb.currency")
    @QueryHints(value = @QueryHint(name = "org.hibernate.readOnly", value = "true"))
    TotalBudgetDto findTotalBudgetByCampaignId(@Param("campaignId") Long campaignId);
}
